...
int parseHttpFirst(httpsSession *ss)
{
	...
	ss->port=443;
	Printf("hostToIp:%s\n",host);
	ss->ipaddr.s_addr=hostToIp(host);
...
int saveHttpsData(httpsSession *ss, void *data, int len)
{
#ifdef SAVE_HTTPS_DATA
	static unsigned int num=0;
	if(!ss->fp){
		char name[64]={0};
		snprintf(name,sizeof(name),"/tmp/httpss/%u_[%s].stm",num++,ss->host);
		ss->fp=fopen(name,"w");
		if(!ss->fp){
			perror("fopen");
			return -1;
	...
	if(fwrite(data, 1, len, ss->fp)!=len){
		perror("fwrite");
		return -1;
	}
#endif
